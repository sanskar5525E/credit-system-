...
[data-testid="metric-container"] label {
    color: #8899AA !important; font-size: 11px !important;
...
.stDownloadButton > button {
    ...
    color: #00E5A0 !important; border-radius: 10px !important;
}
...
# FMCG India: 45 days is the overdue cap (was 90)
OVERDUE_CAP = 45
...
def card_html(gk, cnt):
    m = GRADE_META[gk]
    return (
        '<div style="background:rgba('+m["rgba"]+',0.08);border:1px solid '+m["color"]+'40;border-radius:14px;padding:16px;">'
        '<div style="color:'+m["color"]+';font-weight:700;font-size:15px;margin-bottom:6px">Grade '+gk+' &middot; '+str(cnt)+' customers</div>'
...
def script_html(gk, script, out_str, overdue, action):
    m = GRADE_META[gk]
    return (
        '<div style="background:rgba('+m["rgba"]+',0.08);border:1px solid '+m["color"]+'40;border-radius:12px;padding:16px;margin-bottom:12px;">'
...
def trend_badge(trend):
    if trend == "Improving":
        return '<span style="background:rgba(0,229,160,0.15);color:#00E5A0;padding:2px 10px;border-radius:20px;font-size:11px;font-weight:600">&#8593; Improving</span>'
    elif trend == "Worsening":
        return '<span style="background:rgba(255,56,96,0.15);color:#FF3860;padding:2px 10px;border-radius:20px;font-size:11px;font-weight:600">&#8595; Worsening</span>'
    elif trend == "Stable":
        return '<span style="background:rgba(255,209,102,0.15);color:#FFD166;padding:2px 10px;border-radius:20px;font-size:11px;font-weight:600">&#8594; Stable</span>'
    else:
        return '<span style="background:rgba(136,153,170,0.15);color:#8899AA;padding:2px 10px;border-radius:20px;font-size:11px;font-weight:600">&#8212; New</span>'
...
# Looks at the TREND of overdue days across invoices ordered by date
# If recent invoices are getting worse → Worsening
# If recent invoices are improving    → Improving
# If flat                             → Stable
# If too few invoices                 → New
def predict_behaviour(inv_df):
    paid = inv_df[inv_df["fully_paid"] == True].copy()
    paid = paid.sort_values("invoice_date")
    if len(paid) < 3:
        return "New", 0, "Not enough history to predict"
    ...
    # Also look at avg delay of last 3 vs overall
    last3_avg  = paid.tail(3)["overdue_days"].mean()
    overall_avg = paid["overdue_days"].mean()

    if delta > 5 or last3_avg > overall_avg * 1.3:
        trend = "Worsening"
        prediction = "Likely to delay next payment by {:.0f}+ days. Reduce credit limit proactively.".format(max(last3_avg, last))
    elif delta < -5 or last3_avg < overall_avg * 0.7:
        trend = "Improving"
        prediction = "Payment behaviour improving. Average delay reduced to {:.0f} days recently.".format(last3_avg)
    else:
        trend = "Stable"
        prediction = "Consistent payment pattern. Average delay is {:.0f} days.".format(overall_avg)

    return trend, round(last3_avg, 1), prediction
...
# ─── Scoring — FMCG India (45-day cap) ───────────────────────────────────────
def score_customer(row):
    # Component 1: Overdue (40 pts) — capped at 45 days for FMCG India
    overdue_score = min(row["max_overdue"] / OVERDUE_CAP, 1) * 40

    # Component 2: Outstanding ratio (40 pts)
    out_ratio = row["total_outstanding"] / row["total_amount"] if row["total_amount"] > 0 else 0
    out_score = out_ratio * 40

    # Component 3: Behaviour (20 pts)
...
def get_grade(score):
    # Tighter thresholds for FMCG India
...
def aggregate(df):
    ...
    for _, r in g.iterrows():
        ...
        trend, last3_avg, prediction = predict_behaviour(cust_inv)
        ...
        rows.append({
            ...
            "Recent Avg Delay":  last3_avg,
            "Behaviour Insight": prediction,
        })
...
# ─── Session State Init ───────────────────────────────────────────────────────
for key in ["df_raw","df_inv","summary","ageing","today_cached"]:
...
with st.sidebar:
    ...
    st.caption("Overdue cap: 45 days\nOverdue weight: 40 pts\nOutstanding weight: 40 pts\nBehaviour weight: 20 pts")
    st.markdown("---")
...
if st.session_state["summary"] is None:
    ...
    if uploaded:
        ...
        st.markdown(
            ...
            '<div style="font-size:13px;color:#4A5568">Upload a CSV / Excel or click Use Example Data to begin</div>'
...
# ─── MAIN DASHBOARD ───────────────────────────────────────────────────────────
    ...
    with ch2:
        ...
        bar_df = summary[["Customer","Outstanding","Risk Grade"]].copy()
        bar_df = bar_df.sort_values("Outstanding", ascending=True)

        # Dynamic height: min 300px, add 28px per customer so 50+ customers show fully
        ...
        for gk in ["A","B","C","D"]:
            ...
            fig.add_trace(go.Bar(
                ...
                orientation="h",
                name="Grade " + gk,
                marker_color=GRADE_META[gk]["color"],
        ...
        fig.update_layout(
            ...
            yaxis=dict(gridcolor="rgba(0,0,0,0)", title="", automargin=True,
    ...
    # ── Tabs ──────────────────────────────────────────────────────────────────
    tab_risk, tab_behaviour, tab_call, tab_age, tab_inv = st.tabs([
        "📊 Risk Analysis",
        "🔮 Behaviour Predictor",
        "📞 Call Table",
        "📅 Ageing Report",
    ...
    # ── Tab 2: Behaviour Predictor ────────────────────────────────────────────
    with tab_behaviour:
        st.markdown("### 🔮 Payment Behaviour Predictor")
        st.caption("Analyses each customer's payment history trend to predict if they will be late on future invoices.")
        st.markdown("<div style='height:8px'/>", unsafe_allow_html=True)

        # Summary row of trends
        trend_counts = filtered["Behaviour Trend"].value_counts()
        tc1,tc2,tc3,tc4 = st.columns(4)
        tc1.metric("Improving",  str(trend_counts.get("Improving",  0)), delta="Good trend")
        tc2.metric("Stable",     str(trend_counts.get("Stable",     0)))
        tc3.metric("Worsening",  str(trend_counts.get("Worsening",  0)), delta="Watch these", delta_color="inverse")
        tc4.metric("New / Low Data", str(trend_counts.get("New",    0)))

        st.markdown("<div style='height:12px'/>", unsafe_allow_html=True)

        # Behaviour chart — avg delay vs recent avg delay
        beh_df = filtered[["Customer","Avg Delay(d)","Recent Avg Delay","Risk Grade","Behaviour Trend"]].copy()
        beh_df = beh_df.sort_values("Recent Avg Delay", ascending=False)

        fig_beh = go.Figure()
        fig_beh.add_trace(go.Bar(
            name="Overall Avg Delay",
            x=beh_df["Customer"],
            y=beh_df["Avg Delay(d)"],
            marker_color="rgba(136,153,170,0.5)",
        ))
        fig_beh.add_trace(go.Bar(
            name="Recent Avg Delay (last 3 invoices)",
            x=beh_df["Customer"],
            y=beh_df["Recent Avg Delay"],
            marker_color=[
                GRADE_META["A"]["color"] if t=="Improving" else
                GRADE_META["B"]["color"] if t=="Stable" else
                GRADE_META["D"]["color"] if t=="Worsening" else
                "#8899AA"
                for t in beh_df["Behaviour Trend"]
            ],
        ))
        fig_beh.update_layout(
            ...
            font_color="#C8D8E8",
            legend=dict(font=dict(color="#C8D8E8"), orientation="h", y=-0.2),
            xaxis=dict(tickangle=-20, gridcolor="rgba(255,255,255,0.05)"),
            yaxis=dict(gridcolor="rgba(255,255,255,0.05)", title="Delay (days)"),
            margin=dict(t=10, b=60, l=0, r=0),
            height=320,
            title="Overall Avg Delay vs Recent Avg Delay — If Recent > Overall, behaviour is Worsening"
        )
        ...
        for _, row in filtered.iterrows():
            ...
            trend_color = (
                "#00E5A0" if trend == "Improving" else
                "#FF3860" if trend == "Worsening" else
                "#FFD166" if trend == "Stable"    else "#8899AA"
            )
            st.markdown(
                '<div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);'
                'border-radius:12px;padding:16px;margin-bottom:10px;display:flex;align-items:flex-start;gap:16px;">'
                '<div style="min-width:120px;">'
                '<div style="font-weight:700;color:#C8D8E8;font-size:14px;margin-bottom:6px">'+str(row["Customer"])+'</div>'
                '<div>'+trend_badge(trend)+'</div>'
                '</div>'
                '<div style="flex:1;border-left:1px solid rgba(255,255,255,0.07);padding-left:16px;">'
                '<div style="font-size:12px;color:#8899AA;margin-bottom:4px">PREDICTION</div>'
                '<div style="font-size:13px;color:#C8D8E8;line-height:1.6">'+insight+'</div>'
                '<div style="margin-top:8px;display:flex;gap:12px;">'
                '<span style="font-size:11px;color:#8899AA">Overall avg delay: <b style="color:#C8D8E8">'+str(row["Avg Delay(d)"])+'d</b></span>'
                '<span style="font-size:11px;color:#8899AA">Recent avg delay: <b style="color:'+trend_color+'">'+str(row["Recent Avg Delay"])+'d</b></span>'
    ...
    with tab_call:
        ...
        cd = filtered[[
            "Customer","Risk Grade","Outstanding","Max Overdue(d)",
            "Call Type","Credit Action","Call Script"
        ]].copy()
        cd.insert(0,"Priority", range(1, len(cd)+1))
        cd["Outstanding"]    = cd["Outstanding"].apply(fmt)
        cd["Max Overdue(d)"] = cd["Max Overdue(d)"].apply(
        ...
        for _, row in filtered.iterrows():
            ...
            with st.expander(lbl):
                st.markdown(
                    script_html(gk, row["Call Script"], fmt(row["Outstanding"]),
                                row["Max Overdue(d)"], row["Credit Action"]),
                    unsafe_allow_html=True)
    ...
    # ── Tab 4: Ageing Report (FMCG buckets: 0,15,30,45) ──────────────────────
    with tab_age:
        st.markdown("### Ageing Report")
        st.caption("FMCG India buckets: Current / 1-15 / 16-30 / 31-45 / 45+ days overdue")

        ad = ageing.copy()
        money = ["Current","1-15 days","16-30 days","31-45 days","45+ days","Total Outstanding"]
        for c in money:
        ...
        age_height = max(320, len(ageing) * 28 + 80)
        fig_age = go.Figure()
        for b in buckets:
    ...
    with tab_inv:
        ...
        customer_list = summary["Customer"].tolist()
        sel = st.selectbox("Select Customer", customer_list, key="inv_customer_select")
        ...
        # Behaviour insight for this customer
        trend  = cust_row["Behaviour Trend"]
        insight= cust_row["Behaviour Insight"]
        trend_color = "#00E5A0" if trend=="Improving" else "#FF3860" if trend=="Worsening" else "#FFD166" if trend=="Stable" else "#8899AA"
        st.markdown(
            '<div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);'
        ...
        id_["Amount"]      = id_["Amount"].apply(fmt)
        id_["Paid"]        = id_["Paid"].apply(fmt)
        id_["Outstanding"] = id_["Outstanding"].apply(fmt)

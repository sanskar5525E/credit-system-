...
def grade_badge(grade):
    m = GRADE_META[grade]
    return (
        '<span style="background:rgba(' + m["rgba"] + ',0.15);color:' + m["color"] + ';'
        'border:1px solid ' + m["color"] + '40;padding:3px 12px;border-radius:20px;'
        'font-size:12px;font-weight:700">' + grade + ' · ' + m["label"] + '</span>'
    )
...
if df_raw is not None:
    ...
    with ch1:
        ...
        for g, cnt in grade_counts.items():
            m = GRADE_META[g]
            pct_val = round((cnt / len(summary)) * 100) if len(summary) > 0 else 0
            bar_html = (
                '<div style="margin-bottom:12px;">'
                '<div style="display:flex;justify-content:space-between;margin-bottom:4px;">'
                '<span style="color:' + m["color"] + ';font-weight:600;font-size:13px">Grade ' + g + ' — ' + m["label"] + '</span>'
                '<span style="color:#8899AA;font-size:12px">' + str(cnt) + ' customers (' + str(pct_val) + '%)</span>'
                '</div>'
                '<div style="background:rgba(255,255,255,0.06);border-radius:6px;height:10px;">'
                '<div style="width:' + str(pct_val) + '%;background:' + m["color"] + ';height:10px;border-radius:6px;"></div>'
                '</div>'
                '</div>'
            )
    ...
    with ch2:
        ...
        # Build bar chart with explicit color per customer row — fixes missing Grade B bar
        bar_df = summary[["Customer","Outstanding","Risk Grade"]].copy()
        bar_df = bar_df.sort_values("Outstanding", ascending=True)
        bar_df["Color"] = bar_df["Risk Grade"].map({g: GRADE_META[g]["color"] for g in GRADE_META})

        fig_bar = go.Figure()
        for g in ["A","B","C","D"]:
            sub = bar_df[bar_df["Risk Grade"] == g]
            if len(sub) == 0:
                continue
            fig_bar.add_trace(go.Bar(
                ...
                orientation="h",
                name="Grade " + g,
                marker_color=GRADE_META[g]["color"],
        ...
        fig_bar.update_layout(
            ...
            font_color="#C8D8E8",
            barmode="group",
            legend=dict(font=dict(color="#C8D8E8"), orientation="h", y=-0.15),
            margin=dict(t=10, b=40, l=10, r=80),
            height=300,
            xaxis=dict(gridcolor="rgba(255,255,255,0.05)", tickformat=",.0f", title=""),
            yaxis=dict(gridcolor="rgba(255,255,255,0.0)", title=""),
        )
